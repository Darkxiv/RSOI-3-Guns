...
def user_by_session():
    ...
    user = User.query.filter_by(username=username, pas=pas).first()
    if user:
        return jsonify(id=user.id, username=user.username, telnum=user.telnum, mail=user.mail)
...
def rest_user():
    ...
    if request.method == 'PUT':
        ...
        if user:
            ...
            if telnum and (not testn):
                User.query.filter_by(id=user_id).update({'telnum': telnum})
            if mail and (not testm):
                User.query.filter_by(id=user_id).update({'mail': mail})
            db.session.commit()
            user = User.query.filter_by(id=user_id).first()
            return jsonify(id=user.id, username=user.username, telnum=user.telnum, mail=user.mail)
        #add
    ...
    if request.method == 'DELETE':
        ...
        user = User.query.filter_by(id=user_id).first()
        if user:
            a = User.query.filter_by(id=user_id).delete()
            db.session.commit()
